...
namespace Zu.WebBrowser.Communication {
    public class DebuggerClient : IDebuggerClient {
        ...
        public DebuggerConnection Connection { get { return _connection as DebuggerConnection; } }

        public event EventHandler<JObject> EventMess;
        public event EventHandler<JObject> ResponseMess;
        ...
        public async Task SendRequestAsync(DebuggerCommand command, CancellationToken cancellationToken = new CancellationToken()) {
            ...
            if (!_connection.Connected) throw new Exception("DebuggerConnection not connected");
            try
            {
        ...
        private void HandleResponseMessage(JObject message) {
            ResponseMess?.Invoke(this, message);
            TaskCompletionSource<JObject> promise;
