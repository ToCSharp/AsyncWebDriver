...
namespace Zu.WebBrowser.Commands {
    public abstract class DebuggerCommand {
        private readonly string _commandName;
        public string CommandName => _commandName;

        private static int nextId = 1;
        public DebuggerCommand(int id, string commandName) {
            Id = id;
            if (Id == 0) Id = nextId++;
        ...
        public virtual void ProcessResponse(JToken response) {
            Running = false;  //(bool?)response["running"] ?? false;
