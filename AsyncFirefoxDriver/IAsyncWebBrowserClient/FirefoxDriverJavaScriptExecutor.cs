...
namespace Zu.Firefox
{
    public class FirefoxDriverJavaScriptExecutor : IJavaScriptExecutor
    {
        ...
        {
            var res = await ExecuteScript(script, null, Sandbox /*"defaultSandbox"*/, cancellationToken, args).ConfigureAwait(false);
            return res;
        }
        ...
        {
            var res = await ExecuteAsyncScript(script, null, null /*"defaultSandbox"*/, cancellationToken, args).ConfigureAwait(false);
            return res;
        }

        public async Task<object> ExecuteAsyncScript(string script, string filename = null, string sandbox = "defaultSandbox", CancellationToken cancellationToken = new CancellationToken(), params object[] args)
        {
            await asyncFirefoxDriver.CheckConnected(cancellationToken).ConfigureAwait(false);
            if (asyncFirefoxDriver.ClientMarionette == null)
                throw new Exception("error: no clientMarionette");
            var comm1 = new ExecuteAsyncScriptCommand(script)
            {filename = filename, sandbox = sandbox};
            if (args.Length > 0)
                comm1.Args = args; //.Select(v => v.ToString()).ToArray();
            await asyncFirefoxDriver.ClientMarionette.SendRequestAsync(comm1, cancellationToken).ConfigureAwait(false);
